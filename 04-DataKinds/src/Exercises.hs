...
-- want.
newtype MInt (a :: IntegerMonoid) = M {getInt :: Integer}

-- | b. Write the two monoid instances for 'Integer'.
instance Semigroup (MInt 'Sum) where
  a <> b = M (getInt a + getInt b)
...
data StringAndIntList (stringCount :: Nat) (intCount :: Nat) where
  ...
  ICons :: Int -> StringAndIntList s i -> StringAndIntList x (S i)
...
headSI :: StringAndIntList a b -> Maybe (Either String Int)
headSI SINil = Nothing
headSI (SCons s _) = Just (Left s)
headSI (ICons i _) = Just (Right i)
...
data Constrainable (c :: k -> Constraint) where
  Constrainable :: (c k) => k -> Constrainable c
...
-- on 'HList'.
tailH :: HList (Cons x xs) -> HList xs
tailH (HCons _ xs) = xs
...
data BlogAction (isSafe :: [Role]) where
