...
unpack2EqPair f (EqPair x y) (EqPair z w) = (f x y, f z w)
...
data NestedX input output where
  NestedX :: Component subinput suboutput -> NestedX input output
...
data Child where
  Child :: forall a. (Renderable a) => a -> Child
...
unwrapMBox :: MysteryBox a -> (forall b. MysteryBox b -> r) -> Maybe r
unwrapMBox EmptyBox _ = Nothing
unwrapMBox (IntBox _ i) f = Just (f i)
unwrapMBox (StringBox _ i) f = Just (f i)
unwrapMBox (BoolBox _ i) f = Just (f i)

-- | b. Why do we need a 'Maybe'? What can we still not know?

-- | c. Write a function that uses 'unwrap' to print the name of the next
-- layer's constructor.
innerLayerName = (`unwrapMBox` layerName)
...
vFilter _ VNil g = g VNil
vFilter f (VCons x xs) g
  | f x = vFilter f xs (g . VCons x)
  | otherwise = vFilter f xs g
