...
instance Foldable CanFold where
  ...
  foldr f r (CanFold fa) = foldr f r fa
...
-- pairs?
unpackEqPairs :: (forall a. (Eq a) => a -> a -> r0) -> (forall a. (Eq a) => a -> a -> r1) -> EqPair -> EqPair -> (r0, r1)
unpackEqPairs f0 f1 x y = (unpackEqPair f0 x, unpackEqPair f1 y)
...
unpackNestedX :: (forall i o. Nested input output i o -> r) -> NestedX input output -> r
...
unwrapBox :: (forall a. MysteryBox a -> r) -> MysteryBox a -> Maybe r
unwrapBox _ EmptyBox = Nothing
unwrapBox f (IntBox _ i) = Just (f i)
unwrapBox f (StringBox _ i) = Just (f i)
unwrapBox f (BoolBox _ i) = Just (f i)
...
printNextLayer = fromMaybe "EmptyBox has no inner layer" . unwrapBox f
  where
...
-- >>> filterV odd (VCons 1 (VCons 3 VNil)) lengthV
-- 2

lengthV :: Vector n a -> Int
lengthV VNil = 0
lengthV (VCons _ xs) = 1 + lengthV xs
