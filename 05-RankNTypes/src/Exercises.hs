...
instance Foldable CanFold where
  foldr f acc (CanFold x) = foldr f acc x
...
unwrapBox f (IntBox _ a) = Just $ f a
unwrapBox f (StringBox _ a) = Just $ f a
unwrapBox f (BoolBox _ a) = Just $ f a
...
printNextLayer = fromMaybe "No inner layer left" . unwrapBox f
  where
...
filterV :: (a -> Bool) -> Vector n a -> (forall n. Vector n a -> r) -> r
filterV p VNil f = f VNil
filterV p (VCons x xs) f
  | p x = filterV p xs (f . VCons x)
  | otherwise = filterV p xs f
