...
filterEP f = filter (unpackEqPair f)
...
-- pairs?
unpackEqPairPair :: (forall a. (Eq a) => a -> a -> r) -> (forall b. (Eq b) => b -> b -> s) -> EqPair -> EqPair -> (r, s)
unpackEqPairPair f g x y = (unpackEqPair f x, unpackEqPair g y)
...
unpackNestedX :: (forall subinput suboutput. Component subinput suboutput -> r) -> NestedX input output -> r
unpackNestedX f (NestedX sub) = f sub
...
unwrap :: (forall a. MysteryBox a -> r) -> MysteryBox a -> Maybe r
unwrap _ EmptyBox = Nothing
unwrap f (IntBox _ x) = Just $ f x
unwrap f (StringBox _ x) = Just $ f x
unwrap f (BoolBox _ x) = Just $ f x

-- | b. Why do we need a 'Maybe'? What can we still not know?

-- | c. Write a function that uses 'unwrap' to print the name of the next
-- layer's constructor.
printNextLayer :: MysteryBox a -> String
printNextLayer = fromMaybe "" . unwrap f
  where
    f :: MysteryBox a -> String
    f EmptyBox = "EmptyBox"
    f (IntBox _ _) = "IntBox"
    f (StringBox _ _) = "StringBox"
    f (BoolBox _ _) = "BoolBox"
...
filterV f _ VNil = f VNil
filterV f p (VCons x xs) = if p x then filterV (f . VCons x) p xs else filterV f p xs

-- >>> :set -XStandaloneDeriving
-- >>> deriving instance Show a => Show (Vector n a)
-- >>> filterV show (< 2) (VCons 2 (VCons 3 (VCons 1 VNil)))
-- >>> filterV show (/= 2) (VCons 2 (VCons 3 (VCons 1 VNil)))
-- "VCons 1 VNil"
-- "VCons 3 (VCons 1 VNil)"
