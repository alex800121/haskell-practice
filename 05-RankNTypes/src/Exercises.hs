...
-- pairs?
unpackPairs :: (forall a. (Eq a) => a -> a -> r1) -> (forall a. (Eq a) => a -> a -> r2) -> EqPair -> EqPair -> (r1, r2)
unpackPairs f1 f2 p1 p2 = (unpackEqPair f1 p1, unpackEqPair f2 p2)
...
unwrap (IntBox _ x) f = Just (f x)
unwrap (StringBox _ x) f = Just (f x)
unwrap (BoolBox _ x) f = Just (f x)

-- | b. Why do we need a 'Maybe'? What can we still not know?

-- | c. Write a function that uses 'unwrap' to print the name of the next
-- layer's constructor.
showInner :: MysteryBox a -> String
showInner xs =
  fromMaybe "no inner" $
    unwrap
      xs
...
-- >>> showInner EmptyBox
-- >>> showInner (IntBox undefined EmptyBox)
-- >>> showInner (StringBox undefined (IntBox undefined undefined))
-- >>> showInner (BoolBox undefined (StringBox undefined undefined))
-- "no inner"
-- "empty"
-- "int"
-- "string"
...
filterV p (VCons x xs) f = if p x then filterV p xs (f . VCons x) else filterV p xs f
