...
showConstraints :: ConstrainedList Constraints -> String
showConstraints = foldConstrainedList show

buildMempty :: ConstrainedList Constraints -> ConstrainedList Constraints
buildMempty CNil = CNil
buildMempty (CCons (x :: a) xs) = CCons (mempty :: a) (buildMempty xs)
...
foldMapH :: forall m a xs. (Monoid m, Every ((~) a) xs) => (a -> m) -> HList xs -> m
foldMapH = fold (TCProxy @((~) a))
