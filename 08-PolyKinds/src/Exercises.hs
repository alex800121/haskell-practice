...
newtype Tagged (name :: j) (a :: Type) = Tagged {runTagged :: a}
...
filterV :: (x -> Bool) -> Vector x n -> Sigma (Vector x)
...
filterV p (VCons x xs)
  ...
  where
    xs' = filterV p xs
    cons y (Sigma n ys) = Sigma (SS n) (VCons y ys)
...
data Communication (label :: Label) where
  CComm :: ClientData -> Communication Client
  SComm :: ServerData -> Communication Server
...
serverLog [] = []
serverLog (Sigma SServer (SComm s) : xs) = s : serverLog xs
serverLog (_ : xs) = serverLog xs
...
data Comm where
  SComm' :: ServerData -> Comm
  CComm' :: ClientData -> Comm

serverLog' :: [Comm] -> [ServerData]
serverLog' [] = []
serverLog' (SComm' x : xs) = x : serverLog' xs
serverLog' (_ : xs) = serverLog' xs
