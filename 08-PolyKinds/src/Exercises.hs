...
vFilter :: (a -> Bool) -> Vector a n -> Sigma (Vector a)
vFilter f VNil = Sigma SZ VNil
vFilter f (VCons x xs)
  | f x = (\(Sigma a b) -> Sigma (SS a) (VCons x b)) (vFilter f xs)
...
serverLog [] = []
serverLog (Sigma SServer (CServer x) : xs) = x : serverLog xs
serverLog (Sigma _ _ : xs) = serverLog xs
...
data SimpleComm where
  SimpleClient :: ClientData -> SimpleComm
  SimpleServer :: ServerData -> SimpleComm

simpleServerLog :: [SimpleComm] -> [ServerData]
simpleServerLog (SimpleServer x : xs) = x : simpleServerLog xs
simpleServerLog (_ : xs) = simpleServerLog xs
