...
filterV p (VCons x xs) = (if p x then cons x else id) $ filterV p xs
  where
...
data SLabel (k :: Label) where
  SClient :: SLabel Client
  SServer :: SLabel Server

type instance Sing k = SLabel k
...
data Communication' where
  CClient' :: ClientData -> Communication'
  CServer' :: ServerData -> Communication'

serverLog' :: [Communication'] -> [ServerData]
serverLog' [] = []
serverLog' (CServer' x : xs) = x : serverLog' xs
serverLog' (_ : xs) = serverLog' xs
