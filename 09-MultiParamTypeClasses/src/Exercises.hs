...
instance (f Prelude.~ Applicative) => Wanderable f [] where
  wander _ [] = pure []
...
instance (TypeError (Text "Type not found")) => Pluck x '[] where
  pluck _ = error "type not found"
