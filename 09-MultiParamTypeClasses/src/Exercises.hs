...
class Wanderable (t :: Type -> Type) where
  type ConstraintW t :: (Type -> Type) -> Constraint
  wander :: (Wanderable t, (ConstraintW t) f) => (a -> f b) -> t a -> f (t b)

-- | b. Write a 'Wanderable' instance for 'Identity'.
instance Wanderable Identity where
  type ConstraintW Identity = Functor
...
instance (x < y) => S x < S y where
  ...
  invert FZ = Nothing
  invert (FS n) = SS <$> invert n

-- >>> invert (FS (FS FZ) :: Fin (S (S (S Z)))) :: Maybe (SNat (S Z))
-- >>> invert (FS (FS FZ) :: Fin (S (S (S Z)))) :: Maybe (SNat (S (S Z)))
-- >>> invert (FS (FS FZ) :: Fin (S (S (S (S Z))))) :: Maybe (SNat (S (S (S Z))))
-- Nothing
-- Just (SS (SS SZ))
-- Nothing
...
instance (TypeError (Text "The type " :<>: ShowType x :<>: Text " is not in the HList")) => Pluck x '[] where
...
-- mystery :: String
-- mystery = pluck (HCons (0.5 :: Double) (HCons (3 :: Int) HNil))

-- >>> pluck (HCons (0.5 :: Double) (HCons (3 :: Int) HNil)) :: Int
-- 3

-- >>> pluck (HCons (0.5 :: Double) (HCons (3 :: Int) HNil)) :: Double
-- 0.5

-- >>> pluck (HCons (0.5 :: Double) (HCons (3 :: Int) HNil)) :: String
-- The type [Char] is not in the HList
-- In the expression:
--     pluck (HCons (0.5 :: Double) (HCons (3 :: Int) HNil)) :: String
-- In an equation for `it_aiP0y':
--     it_aiP0y
--       = pluck (HCons (0.5 :: Double) (HCons (3 :: Int) HNil)) :: String
...
instance
  (TypeError (Text "The type " :<>: ShowType x :<>: Text " is no in the Variant")) =>
  Inject x '[]
  where
  inject = error "unreachable"

-- >>> [inject True, inject (3 :: Double), inject "hello"] :: [Variant '[Int, Bool, String]]
-- The type Double is no in the Variant
-- In the expression: inject (3 :: Double)
-- In the expression:
--     [inject True, inject (3 :: Double), inject "hello"] ::
--       [Variant '[Int, Bool, String]]
-- In an equation for `it_aiNjq':
--     it_aiNjq
--       = [inject True, inject (3 :: Double), inject "hello"] ::
--           [Variant '[Int, Bool, String]]

-- >>> [inject True, inject (3 :: Int), inject "hello"] :: [Variant '[Int, Bool, String]]
-- [There (Here True),Here 3,There (There (Here "hello"))]
...
newtype CommentId = CommentId Int deriving (Show, Eq, Ord)
...
data Status = Blocked | Deleted deriving (Show)
...
-- typeclass? Do it!
class Cache (content :: Type) where
  type IdType content :: Type
  type ResultType content :: Type -> Type
  store :: content -> Map (IdType content) content -> Map (IdType content) content
  load :: Map (IdType content) content -> IdType content -> ResultType content content
...
-- GHCi...)
instance Cache User where
  type IdType User = UserId
  type ResultType User = Either Status
  store u@(User i _) = Map.insert i u
  load m i = case m Map.!? i of
    Nothing -> Left Deleted
    Just u -> Right u

instance Cache Comment where
  type IdType Comment = CommentId
  type ResultType Comment = Maybe
  store c@(Comment i _ _) = Map.insert i c
  load = (Map.!?)

oops cache = load cache (UserId (123 :: Int))
-- >>> oops (store (User (UserId 123) "alex") Map.empty)
-- Right (User {id = UserId 123, knownAs = "alex"})

-- >>> oops (store (User (UserId 124) "alex") Map.empty)
-- Left Deleted
