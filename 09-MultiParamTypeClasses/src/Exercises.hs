...
-- | a. Write the class!
class (old Prelude.~ Old new) => Newtype new old where
  type Old new
  wrap :: old -> new
  unwrap :: new -> old
...
add :: (Num old, Newtype new old) => new -> new -> new
...
class (Constrainer t Prelude.~ c) => Wanderable t c where
  type Constrainer t :: (Type -> Type) -> Constraint
  wander :: (c f) => (a -> f b) -> t a -> f (t b)

-- | b. Write a 'Wanderable' instance for 'Identity'.
instance Wanderable Identity Functor where
  type Constrainer Identity = Functor
...
instance
  (TypeError (Text "The type " :<>: ShowType x :<>: Text " is not in the given list")) =>
  Pluck x '[]
  where
...
-- >>> :t [inject True, inject (3 :: Int), inject "Hello"] :: [Variant '[Bool, Int, String]]
-- [inject True, inject (3 :: Int), inject "Hello"] :: [Variant '[Bool, Int, String]] :: [Variant '[Bool, Int, String]]

-- >>> :t [inject True, inject (3 :: Int), inject "Hello"] :: [Variant '[Int, String]]
-- The type Bool is not in the list
-- In the expression: inject True
-- In the expression:
...
class Cacheable c where
  type Cache c :: Type -> Type
  type Id c
  type Result c :: Type -> Type
  store :: c -> Cache c c -> Cache c c
  load :: Cache c c -> Id c -> Result c c
...
instance Cacheable Comment where
  type Cache Comment = Map CommentId
...
-- >>> :type \x -> load x (UserId (123 :: Int))
-- \x -> load x (UserId (123 :: Int)) :: (Id c ~ UserId, Cacheable c) => Cache c c -> Result c c
